...
export default function Stats() {
  ...
  return (
    <section
      id="stats"
      ref={sectionRef}
      className="relative overflow-hidden border-b border-white/5 bg-navy"
    >
      <div className="bg-stripe-w pointer-events-none absolute inset-0 opacity-40" />
      <div className="relative z-[1] mx-auto max-w-[1160px] px-7">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          {STATS.map((stat, i) => (
            <div
              key={stat.label}
              className={`group relative px-5 py-9 text-center transition-colors duration-300 hover:bg-white/5 ${
            ...
            >
              <div className="mx-auto mb-3 flex h-8 w-8 items-center justify-center rounded-lg bg-brand-red/20 [&>svg]:h-4 [&>svg]:w-4 [&>svg]:text-brand-red-soft">
                {stat.icon}
              </div>
              <div className="text-[2.1rem] font-black leading-none tracking-[-0.03em] text-white">
                {values[i]}
              </div>
              <div className="mt-1.5 text-[0.72rem] font-medium uppercase tracking-[0.06em] text-white/50">
                {stat.label}
              </div>
            </div>
          ))}
